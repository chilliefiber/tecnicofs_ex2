...
void run_test(char *server_pipe, int client_id) {
    ...
    char *path = "/f1";
    char buffer[40];
   // char p_buffer_final[100];
    char p_buffer_end_close[100];
   // sprintf(p_buffer_final, "Finished client:%d\n", client_id);
    sprintf(p_buffer_end_close, "Finished client close:%d\n", client_id);
    ...
    write(1, p_buffer_end_close, strlen(p_buffer_end_close)); 
//    assert(tfs_unmount() == 0);
//    write(1, p_buffer_final, strlen(p_buffer_final)); 
}
...
void run_close_function(char *server_pipe, int client_id) {
    ...
    write(1, "Finished closing\n", strlen("Finished closing\n"));
}
