...
int tfs_mount(char const *client_pipe_path, char const *server_pipe_path) {
  ...
  if (unlink(client_pipe_path) != 0 && errno != ENOENT)
    return -1;
  if (mkfifo(client_pipe_path, 0777) !=
  ...
  char msg[PIPE_BUF];
  if (1 + FIFO_NAME_SIZE > PIPE_BUF) // 1 + FIFO_NAME_SIZE is the actual size of
                                     // the message we are going to send
    return -1;
  msg[0] = TFS_OP_CODE_MOUNT;
  strncpy(msg + 1, client_pipe_path,
          FIFO_NAME_SIZE); // No return values are reserved to indicate an error
                           // according to POSIX man page
...
int tfs_unmount() {
  if (!init)
    return -1;
  init = false; // even if it fails, if we called unmount we are allowed to call
                // mount again
...
ssize_t tfs_write(int fhandle, void const *buffer, size_t len) {
  ...
  // in this case there are enough bytes to send the constant size part of the
  // message but not enough for all the bytes we want to write: we write only
  // the bytes that fit inside the buffer that can be atomically written into a
  // named pipe
  if (buf_len > PIPE_BUF) {
...
ssize_t tfs_read(int fhandle, void *buffer, size_t len) {
  ...
  if (ret_code == 0 ||
      ret_code == -1) // if it is 0 no bytes were read but there was no error,
                      // if it is -1 the server sent an error value
    return ret_code;

  // now we know ret_code is a positive value containing the number of bytes the
  // server sent for reading
