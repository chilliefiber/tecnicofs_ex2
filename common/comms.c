...
int write_all(int fd, const void *buffer, size_t len) {
    ...
    while (len) {
        ...
        if (written == -1 && errno == EINTR)
            continue;
        // according to man(2) write, a write to a special file such as a FIFO that returns 0
        // has unspecificed results with regards to errors. According to man(3) write, 
        ...
        // this functions is for communication using pipes and shall be called with PIPE_BUF or less
        // bytes. As such, I consider returning 0 an error. Note that accoridng to the POSIX specification
        // we wouldn't even need the loop for len <= PIPE_BUF
        if (written <= 0) 
            return -1;
        len -= (size_t) written;
        if (len == 0)
